import { D as foldNodeProp, H as tags, V as styleTags, Z as parseMixed, d as LRLanguage, et as EditorView, j as indentNodeProp, m as LanguageSupport, wt as EditorSelection, z as syntaxTree } from "./@codemirror/autocomplete+[...].mjs";
import { i as LRParser, n as ContextTracker, r as ExternalTokenizer } from "./@codemirror/lang-cpp+[...].mjs";
import { n as cssLanguage, t as css } from "./@codemirror/lang-css+[...].mjs";
import { a as typescriptLanguage, i as javascriptLanguage, n as htmlCompletionSource, o as parser$1, r as javascript } from "./@codemirror/lang-html+[...].mjs";
//#region node_modules/@replit/codemirror-lang-svelte/dist/index.js
var scriptText = 145;
...
var tagStart = /*@__PURE__*/ new ExternalTokenizer((input, stack) => {
...
var longExpression = /*@__PURE__*/ new ExternalTokenizer((input) => {
	if (prefixes.includes(input.next)) return;
	...
	const popIfMatch = (match) => {
		...
		if (idx !== -1) while (stack.length > idx) stack.pop();
	};
	for (let pos = 0;; pos++) {
		if (input.next < 0) {
...
var shortExpression = /*@__PURE__*/ new ExternalTokenizer((input) => {
	if (prefixes.includes(input.peek(0))) return;
	const commentHandler = createCommentHandler(input);
	const stringHandler = createStringHandler(input);
	let stack = [];
	const popIfMatch = (match) => {
...
function maybeNest(node, input, tags) {
	let attrs;
	for (let tag of tags) if (!tag.attrs || tag.attrs(attrs || (attrs = getAttrs(node.node.parent, input)))) return { parser: tag.parser };
	return null;
}
		...
		props: [/*@__PURE__*/ indentNodeProp.add({
			...
			"Block": (context) => {
				...
				if (text.startsWith("{/")) {
					...
					if (name === "IfBlock" && text.startsWith("{/if") || name === "EachBlock" && text.startsWith("{/each") || name === "AwaitBlock" && text.startsWith("{/await") || name === "KeyBlock" && text.startsWith("{/key")) return context.lineIndent(context.node.from);
					return null;
				}
...
var autoCloseTags = /*@__PURE__*/ EditorView.inputHandler.of((view, from, to, text) => {
	...
	let changes = state.changeByRange((range) => {
		...
		let name;
		if (around.name === "TagName" || around.name === "ComponentName" || around.name === "SvelteElementName" || around.name === "StartTag") around = around.parent;
